...
to implement a `create` method.
* Any configuration from the [model configuration](model-configuration) is now passed
  in as a dictionary instead of as separate parameters.
* `featurizers` are no longer instantiated outside of policies. Instead, the super class
  `rasa.core.policies.policy.Policy` instantiates the featurizers itself
